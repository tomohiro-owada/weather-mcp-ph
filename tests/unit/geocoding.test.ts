 ...
 * (e.g. "Quezon City, Philippines"). Every city_name lookup (which requests limit=1) therefore
...
vi.mock('axios', () => {
  ...
  // baseURL so a test can tell the providers apart.
...
function nominatimRows(n: number): unknown[] {
  return Array.from({ length: n }, (_, i) => ({
    ...
    lon: `121.${i}`,
    name: `Result ${i}`,
    display_name: `Result ${i}, Metro Manila, Philippines`,
    importance: 0.5,
    type: 'administrative',
    address: {},
  }));
...
/**
 * Route provider requests by baseURL: Nominatim returns `nominatimCount` rows
 * and Open-Meteo returns empty.
 */
...
describe('rfc3986ParamsSerializer', () => {
  it('encodes spaces as %20 (not +) and commas as %2C', () => {
    ...
    expect(rfc3986ParamsSerializer({ q: 'Quezon City, Philippines' })).toBe('q=Quezon%20City%2C%20Philippines');
  });

  it('serializes multiple params and omits undefined/null values', () => {
    const out = rfc3986ParamsSerializer({ q: 'Cebu City', limit: 5, extra: undefined, other: null });
    expect(out).toBe('q=Cebu%20City&limit=5');
  });
});
...
describe('GeocodingService.geocode', () => {
  ...
  it('returns a result for a limit=1 query (regression: city_name lookups)', async () => {
    routeProviders(2);
    const service = new GeocodingService();

    const results = await service.geocode('Quezon City, Philippines', 1);
  ...
  it('requests a result floor of 5 from providers even when the caller asks for 1', async () => {
    ...
    await service.geocode('Quezon City, Philippines', 1);
  ...
  it('slices provider results down to the caller-requested limit', async () => {
    ...
    const results = await service.geocode('Quezon City, Philippines', 2);
  ...
  it('honors a caller limit larger than the floor', async () => {
    ...
    const results = await service.geocode('Quezon City, Philippines', 8);
